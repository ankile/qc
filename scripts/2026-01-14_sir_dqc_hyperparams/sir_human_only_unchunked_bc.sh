...
#SBATCH --gres=gpu:1
#SBATCH --time=2-00:00
#SBATCH --job-name=sir_human_only_unchunked_bc
#SBATCH --output=logs/%x-%j.out
#SBATCH --array=1-3
#SBATCH --requeue

# Unchunked BC on human-only data (matches SIR repo BC baseline)
...
python main.py \
    ...
    --run_group=sir_human_only_unchunked_bc \
    --seed=${SLURM_ARRAY_TASK_ID} \
    --agent.actor_type=best-of-n \
    --agent.actor_num_samples=1 \
    --env_name=sir-square-human_only-low_dim \
    --sparse=True \
    --horizon_length=1 \
